...
## ▶️ DA COSA RIPARTIRE: **Fase 1D — GDPR + rifinitura** (chiude la Fase 1 / MVP)

**Come ripartire:** *"Leggi docs/STATO-LAVORI.md: la Fase 1C è chiusa e mergiata. Partiamo con la 1D (GDPR base): brainstorming + spec + piano."*

La **Fase 1C è completa** (Eventi + RSVP + Album, tutte mergiate su `main`). La prossima e **ultima sotto-fase della Fase 1 (MVP)** è la **1D — GDPR base** (vedi `docs/ROADMAP.md`): **cookie banner + pagine privacy/cookie (struttura)**. **Non c'è ancora spec né piano:** si parte dal **brainstorming** (`superpowers:brainstorming`), poi spec + piano, poi implementazione col metodo subagent-driven.

**Ciò che già si sa sulla 1D:**
- **Caveat GDPR del link Drive (D-172):** i contenuti su Drive sono fuori dal nostro perimetro; la cancellazione di una foto dalla gallery **non** la toglie da Drive → va **dichiarato nella privacy policy** (e rimozione manuale). Questo punto nasce dalla 1C-3 e va coperto qui.
- Roadmap 1D = cookie banner + pagine privacy/cookie (struttura). La **cancellazione account/dati** GDPR completa è in **Fase 4** (vedi ROADMAP), non qui.

**Debiti/follow-up NON bloccanti ereditati (micro-fasi dedicate, non dentro la 1D salvo decisione):**
- Da 1C-3 (deferred-minor dalla review finale): `Modal onClose` inline (pattern ereditato); hidden `<input type=file>` non `disabled` durante busy; `alt=""` sui thumbnail senza caption; lightbox senza `aria-label`/focus-trap/scroll-lock (il `Modal` condiviso invece li ha); due `import type` accorpabili in `page.tsx`; map `mediaAdmin`/`mediaGallery` duplicata; anti-orfano batch non copre il ramo `throw`.
- Debiti di sistema: `revalidatePath` (path non combacianti), **pulizia orfani storage di sistema** (event-covers/vehicles/event-media quando l'admin cancella roba altrui o eventi via cascade), `created_by` degli eventi leggibile da anon via PostgREST.

> ℹ️ **Note dal collaudo 1C-3 (2026-07-28):** admin `mcdevelop03@gmail.com` password reimpostata a `Marsica2026!` via admin API; membro `membro2.test@example.com` → `Membro2026!`. Evento concluso usato: `prova-primo-evento`. Dati di test (media sull'evento, drive_url) sono **locali volatili** (spariscono con `db reset`). I codici di rifiuto storage arrivano come HTTP 400 con `statusCode` 413/415 nel body (versione Supabase locale).
