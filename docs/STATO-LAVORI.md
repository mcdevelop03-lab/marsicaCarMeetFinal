...
- 🎉 **FASE 1E — STAGING CLOUD: COMPLETATA, MERGIATA e CHIUSA** (merge `e0a20c1`, 2026-08-04). `main` pushato su origin, **branch `feat/fase1e-staging-cloud` eliminato** (locale e remoto), branch di produzione Netlify **spostato su `main`** dall'utente. **C'è un cliente che deve provare e approvare il sito**: è il motivo per cui esiste questa fase.
  - 🔴 **DEPLOY NETLIFY FERMI dal 2026-08-04: i credits del piano Free sono esauriti.** Messaggio del dashboard: *"Your published sites are still live, but production deploys are paused"*. **Il sito resta online** (serve l'ultima build riuscita, il cliente non se ne accorge) ma **ogni push non produce più niente, senza alcun errore in git**.
    - **Causa: build sprecate.** Ogni push sul branch di produzione ricompila tutto, e il 2026-08-04 sono stati fatti ~10 push di **sola documentazione**. ✅ **Risolto** con una regola `ignore` in [`netlify.toml`](../netlify.toml), provata sui commit veri: salta la build se il commit tocca solo `docs/`, `.md` o il ledger. **Regola da tenere: commit di documentazione e di codice separati.**
    - ⏳ **Si sbloccano al prossimo ciclo di fatturazione** (o pagando 9 $/mese). **Fino ad allora, qualunque modifica al sito resta invisibile online**: se qualcosa "non si aggiorna", è questo e non un guasto.
    - ⏸️ **In sospeso per colpa di questo:** il commit `6d80cab` (correzioni alla privacy) **non è online**, e **non si è potuto verificare che Netlify costruisca da `main`** invece che dal vecchio branch — serviva proprio un deploy per dimostrarlo. **Prima prova utile al primo deploy che riuscirà.** Spec: [`superpowers/specs/2026-07-30-fase1e-staging-cloud-design.md`](./superpowers/specs/2026-07-30-fase1e-staging-cloud-design.md) · Piano: [`superpowers/plans/2026-07-30-fase1e-staging-cloud.md`](./superpowers/plans/2026-07-30-fase1e-staging-cloud.md) · Ledger: [`../.superpowers/sdd/2026-07-30-fase1e-staging-cloud/progress.md`](../.superpowers/sdd/2026-07-30-fase1e-staging-cloud/progress.md).
