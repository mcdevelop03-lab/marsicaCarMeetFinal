...
export default function Header({ isAuthenticated }: { isAuthenticated: boolean }) {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-background/90 backdrop-blur-md border-b border-white/5 px-6 md:px-12 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <nav className="hidden lg:flex items-center gap-1 font-mono text-xs font-bold uppercase tracking-wider">
              {links.map((l) => (
                <Link
                  key={l.href}
                  href={l.href}
              ...
              {isAuthenticated ? (
                <form action={logout}>
                  <button
                    type="submit"
                    className="px-4 py-2 rounded text-accent-red hover:text-white transition-colors font-mono text-xs font-bold uppercase tracking-wider"
                  >
                    {t("logout")}
                  </button>
                </form>
      ...
      {open && (
        <MobileMenu links={links} isAuthenticated={isAuthenticated} onClose={() => setOpen(false)} />
      )}
    </>
  );
}
